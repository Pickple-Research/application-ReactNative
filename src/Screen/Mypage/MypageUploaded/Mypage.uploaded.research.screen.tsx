...
export type MypageUploadedResearchScreenProps = {};

/**
 * 마이페이지 - 작성한 글 - 리서치 페이지입니다
 * @author 현웅
 */
export function MypageUploadedResearchScreen() {
  ...
  return (
    <Container style={globalStyles.screen__horizontalPadding}>
      <UploadedResearchList<React.ElementType>
        data={uploadedResearches}
...
/**
 * 업로드한 리서치에 대해 전체 | 진행중 | 마감 필터를 적용하는 리스트 헤더입니다.
 * @author 현웅
 */
function FlatListHeader({ style }: { style?: StyleProp<ViewStyle> }) {
  const { researchFilter, setResearchFilter } = useMypageUploadedScreenStore(
    state => ({
...
/**
 * 업로드한 리서치를 선택한 필터에 따라 나누어 보여줍니다.
 * @author 현웅
 */
function FilteredResearchItem({ research }: { research: ResearchSchema }) {
  const researchFilter = useMypageUploadedScreenStore(
...
function EmptyScreen() {
  return (
    <EmptyScreen__Container>
      <EmptyScreen__Text>게시한 글이 없습니다</EmptyScreen__Text>
    </EmptyScreen__Container>
  );
}
...
const UploadedResearchList = styled.FlatList``;
