...
export function MypageParticipatedVoteScreen() {
  ...
  return (
    <Container style={globalStyles.screen__horizontalPadding}>
      <ParticipatedVoteList<React.ElementType>
...
/**
 * 참여한 투표에 대해 전체 | 진행중 | 마감 필터를 적용하는 리스트 헤더입니다.
 * @author 현웅
 */
function FlatListHeader({ style }: { style?: StyleProp<ViewStyle> }) {
  const { voteFilter, setVoteFilter } = useMypageParticipatedScreenStore(
    state => ({
...
/**
 * 참여한 투표를 선택한 필터에 따라 나누어 보여줍니다.
 * @author 현웅
 */
function FilteredVoteItem({ vote }: { vote: VoteSchema }) {
  const voteFilter = useMypageParticipatedScreenStore(
    state => state.voteFilter,
  );
...
function EmptyScreen() {
  return (
    <EmptyScreen__Container>
      <EmptyScreen__Text>참여한 투표글이 없습니다</EmptyScreen__Text>
    </EmptyScreen__Container>
  );
}
...
const ParticipatedVoteList = styled.FlatList``;
